...
class HVAE(BaseVAE):
        ...
        **kwargs,
        ...
        for h_dim in hidden_dims:
            modules.append(
                nn.Sequential(
                    nn.Conv2d(
                        channels, out_channels=h_dim, kernel_size=3, stride=2, padding=1
                    ),
    ...
    def encode_z2(self, input: torch.tensor) -> list[torch.tensor]:
        """
    ...
    def encode_z1(self, input: torch.tensor, z2: torch.tensor) -> list[torch.tensor]:
    ...
    def loss_function(self, *args, **kwargs) -> dict:
        ...
        z1_kld = torch.mean(
            -0.5 * torch.sum(1 + z1_log_var - z1_mu**2 - z1_log_var.exp(), dim=1), dim=0
        )
        z2_kld = torch.mean(
            -0.5 * torch.sum(1 + z2_log_var - z2_mu**2 - z2_log_var.exp(), dim=1), dim=0
        )

        z1_p_kld = torch.mean(
            -0.5
            * torch.sum(
                1 + z1_p_log_var - (z1 - z1_p_mu) ** 2 - z1_p_log_var.exp(), dim=1
            ),
            dim=0,
        )

        torch.mean(-0.5 * (z2**2), dim=0)

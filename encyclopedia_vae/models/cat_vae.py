...
class CategoricalVAE(BaseVAE):
    def __init__(
        ...
        **kwargs,
        ...
        self.fc_z = nn.Linear(
            hidden_dims[-1] * 4, self.latent_dim * self.categorical_dim
        )
    ...
    def encode(self, input: torch.tensor) -> list[torch.tensor]:
        ...
        :param input: (torch.tensor) Input tensor to encoder [B x C x H x W]
        :return: (torch.tensor) Latent code [B x D x Q]
        """
    ...
    def decode(self, z: torch.tensor) -> torch.tensor:
        ...
        :param z: (torch.tensor) [B x D x Q]
    ...
    def reparameterize(self, z: torch.tensor, eps: float = 1e-7) -> torch.tensor:
        """
        Gumbel-softmax trick to sample from Categorical Distribution
        :param z: (torch.tensor) Latent Codes [B x D x Q]
    ...
    def sample(self, num_samples: int, current_device: int, **kwargs) -> torch.tensor:
        ...
        np_y = np.reshape(
            np_y, [M // self.latent_dim, self.latent_dim, self.categorical_dim]
        )
        z = torch.from_numpy(np_y)

        # z = self.sampling_dist.sample((num_samples * self.latent_dim, ))
        z = z.view(num_samples, self.latent_dim * self.categorical_dim).to(
            current_device
        )
